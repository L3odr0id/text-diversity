...
class CustomFilteredFilesList(FilteredFilesList):
    ...
    def save(self, knee: Knee):
        ...
        logging.info(
            f"[KNEEBOX] knee: {self.knee_point}. list_to_save_len: {len(self.list_to_save)}"
        )
...
def main():
    ...
    for i in range(args.experiment_rounds):
        result = _call_experiment(args, file_paths, i)
        if result is None:
            continue
        ...
        logging.info(
            f"Experiment. Finished round {finished_rounds} / {args.experiment_rounds}"
        )
        produce_artifacts(
